      ...
      <div className="d-flex align-items-center mb-4 alert alert-primary">
        ...
        />
          ...
          FABRIC is funded by NSF grants {nsfAwardNumbers.map((number, index) => 
            <span key={`nsf-award-${number}`}><a href={`https://www.nsf.gov/awardsearch/showAward?AWD_ID=${number}&HistoricalAwards=false`} target="_blank" rel="noopener noreferrer">CNS-{number}</a>{index !== nsfAwardNumbers.length -1 && ", "}</span>)
          }.
          </div>
      </div>
