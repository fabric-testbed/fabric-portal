...
export default function FundingOpportunities(){
  return (
    <div className="container static-page pb-5">
      ...
      {
        parsedSolicitations().current.length === 0 && <div className="alert alert-primary mb-5" role="alert">
          There are no current funding opportunities at this time. Please check back later.
        </div>
      }
      {
        parsedSolicitations().current.length > 0 && <table className="table table-striped mb-5">
        ...
        <tbody>
          {
            parsedSolicitations().current.map((opportunity, index) => <tr key={`fab-core-member-${index}`}>
              ...
              <td>
                <a href={"hh"} target="_blank" rel="noreferrer">
                  <LinkIcon size={16} />
                </a>
              </td>
        ...
        <tbody>
          {
            parsedSolicitations().expired.map((opportunity, index) => <tr key={`fab-core-member-${index}`}>
              <th>{opportunity.name}</th>
              <td><a href={opportunity.url} target="_blank" rel="noreferrer"><LinkIcon size={16} /></a></td>
              <td>{opportunity.displayDate}</td>
            </tr>
          )
          }
