...
export function getProjects(type, offset, limit, searchQuery, searchSet) {
  ...
  const searchSetParam = searchSet ? `&search_set=${searchSet}` : "";
  if (type === "myProjects") {
    if (!searchQuery) {
      return http.get(`${apiEndpoint}?offset=${offset}&limit=${limit}&sort_by=created_time&order_by=desc&person_uuid=${userID}`);
    } else {
      return http.get(`${apiEndpoint}?search=${searchQuery}&offset=${offset}&limit=${limit}&sort_by=created_time&order_by=desc&person_uuid=${userID}${searchSetParam}`);
    }
  } else if (type === "allProjects") {
    if (!searchQuery) {
      return http.get(`${apiEndpoint}?offset=${offset}&limit=${limit}&sort_by=created_time&order_by=desc`);
    } else {
      return http.get(`${apiEndpoint}?search=${searchQuery}&offset=${offset}&limit=${limit}&sort_by=created_time&order_by=desc${searchSetParam}`);
    }
  }
}
