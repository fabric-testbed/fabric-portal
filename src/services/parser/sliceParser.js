export default function parseSlice(abqm) {
  ...
  // *************************************************
  // *************************************************
  // Create all 'ConnectionPoint' with type "ServicePort".
  nodes.forEach(node => {
    if (node.Class === "ConnectionPoint" && node.Type === "ServicePort") {
      const data = {
        id: node.id,
        label: `${node.id}.cp`,
        type: "roundrectangle",
  ...
  // *************************************************
  // *************************************************
  // Create NetworkService with type "L2Bridge"
  nodes.forEach(node => {
  ...
  links.forEach(link => {
    ...
    if (link.Class === "connects") {
      // if NetworkService to CP, then add CP node inside NS's parent, don't add eage.
